...
pub fn load_workflow(name: &str) -> Result<Workflow, WorkflowError> {
    ...
    let content = fs::read_to_string(&workflow_path).map_err(|e| WorkflowError::IoError(e))?;
...
fn find_workflow_file(name: &str) -> Result<PathBuf, WorkflowError> {
    ...
    if local_path.exists() {
        println!(
            "Found workflow in local directory: {}",
            local_path.display()
        );
    ...
    if let Some(global_dir) = get_home_workflows_path() {
        ...
        if global_path.exists() {
            println!(
                "Found workflow in home directory: {}",
                global_path.display()
            );
...
pub fn ensure_workflows_directory() -> Result<(), WorkflowError> {
    ...
    if !local_dir.exists() {
        println!(
            "Creating local workflows directory: {}",
            local_dir.display()
        );
        fs::create_dir_all(&local_dir).map_err(|e| WorkflowError::IoError(e))?;
    }
    ...
    if let Some(global_dir) = get_home_workflows_path() {
        if !global_dir.exists() {
            println!(
                "Creating home workflows directory: {}",
                global_dir.display()
            );
            fs::create_dir_all(&global_dir).map_err(|e| WorkflowError::IoError(e))?;
        }
    }
