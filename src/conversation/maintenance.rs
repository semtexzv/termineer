...
pub fn is_empty_content(content: &Content) -> bool {
    match content {
        ...
        Content::RedactedThinking { data } => data
            .as_ref()
...
mod tests {
    ...
    fn test_is_empty_content_text() {
        // Empty text
        let empty = Content::Text { text: "".to_string() };
        assert!(is_empty_content(&empty));
        
        // Whitespace-only text
        let whitespace = Content::Text { text: "   \n  \t  ".to_string() };
    ...
    fn test_is_empty_content_thinking() {
        // Thinking with None
        let none_thinking = Content::Thinking { thinking: None, signature: None };
