    ...
    if !silent_mode {
        match &command {
            InputCommand::Click { x, y, window_id, button, double } => {
                ...
                crate::bprintln!("🖱️ Sending {}{}click at ({},{}) to window '{}'...", 
                    double_str, button_str, x, y, window_id);
            },
            ...
            InputCommand::KeyPress { key, modifiers, window_id } => {
                crate::bprintln!("⌨️ Sending key {} with modifiers {} to window '{}'...", 
                    key, modifiers.join("+"), window_id);
            },
            InputCommand::Sequence { actions, window_id } => {
                crate::bprintln!("🤖 Executing {} input actions on window '{}'...", 
                    actions.len(), window_id);
