...
impl UIElement {
    ...
    pub fn to_xml(&self) -> Result<String, String> {
        ...
        let xml_string = String::from_utf8(result).map_err(|e| format!("XML encoding error: {}", e))?;
    ...
    fn write_xml_element<W: std::io::Write>(&self, writer: &mut Writer<W>, depth: usize) -> Result<(), String> {
        // Create element
        ...
        // Write element start
        writer.write_event(Event::Start(elem)).map_err(|e| format!("XML write error: {}", e))?;
        ...
        for (key, value) in &self.attributes {
            ...
            writer.write_event(Event::Start(attr_elem))
                .map_err(|e| format!("XML write error: {}", e))?;
        ...
        // Close element
        writer.write_event(Event::End(BytesEnd::new("UIElement")))
            .map_err(|e| format!("XML write error: {}", e))?;
...
impl UIWindow {
    ...
    pub fn to_xml(&self) -> Result<String, String> {
        ...
        // Root element
        writer.write_event(Event::Start(BytesStart::new("UITree")))
            .map_err(|e| format!("XML write error: {}", e))?;
        ...
        writer.write_event(Event::Start(window_elem))
            .map_err(|e| format!("XML write error: {}", e))?;
        ...
        writer.write_event(Event::End(BytesEnd::new("Window")))
            .map_err(|e| format!("XML write error: {}", e))?;
        
        // Close root element
        writer.write_event(Event::End(BytesEnd::new("UITree")))
            .map_err(|e| format!("XML write error: {}", e))?;
        
        let result = writer.into_inner().into_inner();
        let raw_xml = String::from_utf8(result).map_err(|e| format!("XML encoding error: {}", e))?;
        ...
        for c in xml.chars() {
            match c {
                '<' => {
                    ...
                    let is_closing = xml.chars().skip(xml.len() - formatted.len()).next() == Some('/');
                    if is_closing {
                ...
                '>' => {
                    ...
                    if in_declaration {
                        ...
                        if !is_self_closing && !xml.chars().skip(xml.len() - formatted.len()).next().unwrap_or(' ').is_whitespace() {
