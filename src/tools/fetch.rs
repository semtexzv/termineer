...
async fn summarize_text(text: &str, length: &SummaryLength) -> Result<String, String> {
    ...
    let message = Message::text("user", prompt, MessageInfo::User);

    // Send to the model - use 1000 tokens max for summaries to keep them concise
    let max_tokens = Some(1000);
    let response = match backend
        .send_message(&[message], None, None, None, None, max_tokens)
        .await
    {
            ...
            Some(length) => {
                match summarize_text(&processed_text, &length).await {
                    Ok(summary) => {
                        ...
                        if !silent_mode {
                            // Print fetch success and summary directly
                            crate::btool_println!(
                                "fetch",
                                "{}🌐 Fetch:{} {} - Content summarized successfully",
                                FORMAT_BOLD,
                                FORMAT_RESET,
                                url
                            );
                    ...
                    Err(err) => {
                        ...
                        if !silent_mode {
                            // Use buffer-based printing
                            crate::bwarning_println!("Failed to summarize content: {}", err);
                            crate::btool_println!(
                                "fetch",
                                "{}🌐 Fetch:{} {} - Returning full content instead",
                                FORMAT_BOLD,
                                FORMAT_RESET,
                                url
                            );
